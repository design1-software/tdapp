...
# Tests never call the real Anthropic API — brief_service.call_claude is patched with a mock.
...
class TestGenerateBrief:
    # 📘 @patch replaces brief_service.call_claude with a MagicMock for the duration of the test.
    # We control exactly what "Claude" returns, so tests are fast and free.
    @patch("brief_service.call_claude", return_value=VALID_BRIEF_JSON)
...
class TestBriefErrorHandling:
    ...
    @patch("brief_service.call_claude", return_value="this is not json {{{")
    ...
    @patch("brief_service.call_claude", return_value=json.dumps({
        "situation": "ok",
